...
export class DatabaseStorage implements IStorage {
  ...
  async updateProductStock(slug: string, stock: number): Promise<Product | undefined> {
    const [product] = await db
      .update(products)
      .set({ stock })
      .where(eq(products.slug, slug))
  ...
  async updateProductName(slug: string, name: string): Promise<Product | undefined> {
    const [product] = await db
      .update(products)
      .set({ name })
      .where(eq(products.slug, slug))
  ...
  async updateProductStoreCode(slug: string, storeCode: string | null): Promise<Product | undefined> {
    const [product] = await db
      .update(products)
      .set({ storeCode })
      .where(eq(products.slug, slug))
